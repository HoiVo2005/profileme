...
export default function DynamicSidebar() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (d.error) {
        console.error("[Supabase] Không tải được documents, dùng dữ liệu mẫu:", d.error.message);
        setDocs(getFallbackDocuments());
      } else {
      ...
      if (s.error) {
        console.error("[Supabase] Không tải được skills, dùng dữ liệu mẫu:", s.error.message);
        setSkills(getFallbackSkills());
      } else {
